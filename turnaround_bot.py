...
try:
    ...
    df_cap_filtered = df_krx[df_krx['Marcap'] >= 200000000000]
    ...
    for item in passed_stocks:
        ...
        prompt = f"기업명: {item['name']}\n공시: {news_list}\n바닥 탈출 및 턴어라운드 가능성을 1~10점 사이로 평가하고 한 줄 요약하세요.\n형식(반드시 2줄):\n반등 스코어: 8점\nAI 요약: 내용"
        try:
            res = model.generate_content(prompt)
            lines = [line.replace('*', '').strip() for line in res.text.strip().split('\n') if line.strip()]
            score_line = next((line for line in lines if "스코어" in line), "반등 스코어: 평가 불가")
